pipeline {
    ...
    stages {
            ...
            steps {
                ...
                script {
                    if ( env.BRANCH_NAME == 'master' ) {
                        containerVersion = getVersionFromContainer("liatrio/ldop-logstash:${env.BRANCH_NAME}")
                        failIfVersionExists("liatrio","ldop-logstash",containerVersion)
                        sh "docker build -t liatrio/ldop-logstash:${containerVersion} ."
                    }
                }
            }
            post {
        ...
        stage('Integration-test') {
            ...
            steps {
                ...
                withCredentials([[
                    ...
                    testSuite("ldop-logstash", "${env.BRANCH_NAME}", DIR)
        ...
        stage('Push to dockerhub') {
            ...
            steps {
                script { CHANGED = "NO" }
                sh "docker tag liatrio/ldop-logstash:${env.BRANCH_NAME} liatrio/ldop-logstash:latest"
                sh "docker push liatrio/ldop-logstash:latest"
                script {
                    if ( env.BRANCH_NAME == 'master' )
                        sh "docker push liatrio/ldop-logstash:${containerVersion}"
                }
            }
            post {
                success { script { STATUS = "SUCCESS" } }
